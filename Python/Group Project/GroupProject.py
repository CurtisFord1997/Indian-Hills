...
#This game take user input word and it display funny stuff depending on the user input
#and it has 6 lavel for user with different word and discriptions
#Natnail Section
class Game:
    ...
    #Read the file and comper the data that the user enterd
    #If the user enterd different save key it contains
    def newGame(self):
...
#Curtis Section
#creates a screen with objects from each level and an inventory, updates the respective lists, 
#and refreshes the text on the buttons depending on what item was selected.
class LevelPane(object):
    ...
    #creates a level  based on current level param, and loops through the set of items in the level object to 
    #rename the option buttons
    ...
    #loops through the set of items in theInnentory item in the Game object to rename the option buttons
    ...
    #is activated by clicking one of the options, adds the selection to the game inventory list, updates the inventory, and updates the options
...
#Justin's portion
#is a frame that just shows the results of what the person chose
